...
rss_check = ad.rss.core.RssCheck()
global timeIndex


def getUnstructuredSettings():
...
def get_object_vehicle_dynamics():
    advRssDynamics = ad.rss.world.RssDynamics()
    advRssDynamics.responseTime = ad.physics.Duration(0.75)
    advRssDynamics.maxSpeedOnAcceleration = ad.physics.Speed(0.)
    advRssDynamics.alphaLat.accelMax = ad.physics.Acceleration(0.79)
    advRssDynamics.alphaLat.brakeMin = ad.physics.Acceleration(-0.1) # This value is from nowhere, don't know
    advRssDynamics.alphaLon.accelMax = ad.physics.Acceleration(0.76)
    advRssDynamics.alphaLon.brakeMax = ad.physics.Acceleration(-0.66)
    advRssDynamics.alphaLon.brakeMinCorrect = ad.physics.Acceleration(-0.33) # Unsure where to get this value from
    advRssDynamics.alphaLon.brakeMin = ad.physics.Acceleration(-0.33)
    advRssDynamics.lateralFluctuationMargin = ad.physics.Distance(0.) #Unsure 
    advRssDynamics.unstructuredSettings = getUnstructuredSettings()
    return advRssDynamics

def get_ego_vehicle_dynamics():
    egoRssDynamics = ad.rss.world.RssDynamics()
    egoRssDynamics.responseTime = ad.physics.Duration(0.75)
    egoRssDynamics.maxSpeedOnAcceleration = ad.physics.Speed(0.)
    egoRssDynamics.alphaLat.accelMax = ad.physics.Acceleration(2.79)
    egoRssDynamics.alphaLat.brakeMin = ad.physics.Acceleration(-0.1) # This value is from nowhere, don't know
    egoRssDynamics.alphaLon.accelMax = ad.physics.Acceleration(2.3)
    egoRssDynamics.alphaLon.brakeMax = ad.physics.Acceleration(-2.7)
    egoRssDynamics.alphaLon.brakeMinCorrect = ad.physics.Acceleration(-1.78) # Unsure where to get this value from
    egoRssDynamics.alphaLon.brakeMin = ad.physics.Acceleration(-1.78)
    egoRssDynamics.lateralFluctuationMargin = ad.physics.Distance(0.) #Unsure 
    egoRssDynamics.unstructuredSettings = getUnstructuredSettings()
    return egoRssDynamics

# def initialize_object(lon, lat, yawAngle, resultObject):
...
def objectToRssObjectData(object, rss_dynamics, type):
    rss_object = ad.rss.map.RssObjectData()

    if type == 'ego':
        rss_object.id = 1
        rss_object.type = ad.rss.world.ObjectType.EgoVehicle
        rss_object.matchObject.enuPosition.dimension.length = ad.physics.Distance(5.1816)
        rss_object.matchObject.enuPosition.dimension.width = ad.physics.Distance(2.0066)
        rss_object.matchObject.enuPosition.dimension.height = ad.physics.Distance(1.5)
    else:
        rss_object.id = 2
        rss_object.type = ad.rss.world.ObjectType.Pedestrian
        rss_object.matchObject.enuPosition.dimension.length = ad.physics.Distance(0.4)
        rss_object.matchObject.enuPosition.dimension.width = ad.physics.Distance(0.7)
        rss_object.matchObject.enuPosition.dimension.height = ad.physics.Distance(1)

    rss_object.speed = ad.physics.Speed(object.speed)
    #rss_object.yaw = ad.physics.AngularVelocity(object.heading)
    rss_object.yawRate = ad.physics.AngularVelocity(0.)
    rss_object.steeringAngle = ad.physics.Angle(0.)
    rss_object.rssDynamics = rss_dynamics

    rss_object.matchObject.enuPosition.heading = ad.map.point.createENUHeading(object.heading)
    rss_object.matchObject.enuPosition.centerPoint.x = ad.map.point.ENUCoordinate(object.center.x)
    rss_object.matchObject.enuPosition.centerPoint.y = ad.map.point.ENUCoordinate(object.center.y)
    rss_object.matchObject.enuPosition.centerPoint.z = ad.map.point.ENUCoordinate(0)
    

    return rss_object
    
# def initialize_ego_vehicle(egoObject):
#         # laneId: offset 240151:0.55
#         initialize_object(ad.map.point.Longitude(8.00125444865324766),
...
#         egoRoute = ad.map.route.planRoute(
#             egoObject.mapMatchedBoundingBox.referencePointPositions[
#                 ad.map.match.ObjectReferencePoints.Center][0].lanePoint.paraPoint,
#             positionEndGeo)
        
def timeUpdate(egoObject, advObject, timeIndex):
    egoObjectData = objectToRssObjectData(egoObject, get_ego_vehicle_dynamics(), 'ego')
    RssSceneCreator =ad.rss.map.RssSceneCreation(timeIndex, get_ego_vehicle_dynamics())
    advObjectData = objectToRssObjectData(advObject, get_object_vehicle_dynamics(), 'pedestrian')
    egoRouteInput = ad.rss.map.route.FullRoute()
    restrict_speed_limit_mode = ad.rss.map.RssSceneCreation.RestrictSpeedLimitMode.IncreasedSpeedLimit10
    greenTrafficLights = ad.map.landmark.LandmarkIdSet()
    mode = ad.rss.map.RssMode.Unstructured

    RssSceneCreator.appendScenes(egoObjectData,
                                      egoRouteInput,
                                      advObjectData,
    ...
    world_model = RssSceneCreator.getWorldModel()
...
columns_dict = {
    ...
    'SEI_TTC':  [],
    'SEI_MM':   [],
    'SEI_RSS_PED':  [],     # Pseudo Pedestrian RSS Calculation
...
class vehicle:
    ...
    def __init__(self, x: float, y: float, heading: float, dimensions: [float], type: str):
    ...
    def get_mm_bbox(self):
        ...
        if self.type == 'vehicle':
            if self.speed < 2:
                ...
                bbox_distance = 0.046 * self.speed + 0.131
        elif self.type == 'pedestrian':
            if self.speed < 1.5:
                bbox_distance = 0.2
            else:
                bbox_distance = 0.188 * self.speed - 0.076
        ...
        # Rotate to match the heading angle
        return affinity.rotate(rect.buffer(bbox_distance), angle, 'centroid')
    ...
    def get_rss_ped_bbox(self):
        ...
        return affinity.rotate(bbox, angle - 90, self.center)
    ...
    def get_rss_vehicle_bbox(self):
        ...
        if self.speed > 3:
            bbox = affinity.scale(bbox, xfact=self.speed/20, yfact=self.speed/20, origin=self.center)
        else:
...
def calculate_who_is_in_front(vut, challenger):
    new_VUT = affinity.rotate(vut.get_bbox(), (vut.heading - 90) * -1, vut.center)
    new_challenger = affinity.rotate(challenger.get_bbox(), (vut.heading - 90) * -1, vut.center)

    if shapely.centroid(new_challenger).y > shapely.centroid(new_VUT).y:
        return 'challenger'
    else:
        return 'VUT'

# def calculate_delta_pos_lon(data_df, vut, challenger):
#     '''    
#     calculate delta in position for car following situations
#     delta_d = d_lead - d_fol
#     '''
#     data_df['delta_pos_lon'] = [None] * len(data_df['challenger y'])

#     for index, row in data_df.iterrows():
#         #if (data_df['challenger y'][index] > data_df['VUT y'][index]):
#         if (calculate_who_is_in_front(vut, challenger) == 'challenger'):
#             lead_x = data_df['challenger x'][index]
#             lead_y = data_df['challenger y'][index]
#             follow_x = data_df['VUT x'][index]
#             follow_y = data_df['VUT y'][index]
#         else:
#             lead_x = data_df['VUT x'][index]
#             lead_y = data_df['VUT y'][index]
#             follow_x = data_df['challenger x'][index]
#             follow_y = data_df['challenger y'][index]

#         # delta_pos = data_df['d_lon']
#         # data_df['delta_pos_lon'] = data_df['d_lon']

#         dx = lead_x - follow_x
#         dy = lead_y - follow_y

#         delta_dist = (dx**2 + dy**2)**0.5
        
#         data_df['delta_pos_lon'][index] = delta_dist

def calculate_delta_pos_lon(vut, challenger):
    '''    
    calculate delta in position for car following situations
    delta_d = d_lead - d_fol
    '''
    #delta_pos = [None] * len(data_df['challenger y'])

    #for index, row in data_df.iterrows():
        #if (data_df['challenger y'][index] > data_df['VUT y'][index]):
    if (calculate_who_is_in_front(vut, challenger) == 'challenger'):
        lead_x = challenger.center.x
        lead_y = challenger.center.y
        follow_x = vut.center.x
        follow_y = vut.center.y
    else:
        lead_x = vut.center.x
        lead_y = vut.center.y
        follow_x = challenger.center.x
        follow_y = challenger.center.y
...
# def calculate_delta_vel_lon(data_df, vut, challenger):
#     '''
#     calculate delta in velocity
#     delta_v = v_fol - v_lead
#     '''

#     data_df['delta_vel_lon'] = [None] * len(data_df['challenger y'])
    
#     for index, row in data_df.iterrows():
#         #if (data_df['challenger y'][index] > data_df['VUT y'][index]):
#         if (calculate_who_is_in_front(vut, challenger) == 'challenger'):
#             lead_vel = data_df['challenger sp'][index]
#             follow_vel = data_df['VUT sp'][index]
#         else:
#             lead_vel = data_df['VUT sp'][index]
#             follow_vel = data_df['challenger sp'][index]

#         delta_vel = follow_vel - lead_vel
#         data_df['delta_vel_lon'][index] = delta_vel

def calculate_delta_vel_lon(vut, challenger):
    '''
    calculate delta in velocity
    delta_v = v_fol - v_lead
    '''
    ...
    #for index, row in data_df.iterrows():
        #if (data_df['challenger y'][index] > data_df['VUT y'][index]):
    if (calculate_who_is_in_front(vut, challenger) == 'challenger'):
        lead_vel = challenger.speed
        follow_vel = vut.speed
    else:
        lead_vel = vut.speed
        follow_vel = challenger.speed
...
# def calculate_delta_acc_lon(data_df, vut, challenger):
#     '''
#     calculate delta in acceleration
#     delta_acc_lon = a_fol - a_lead
#     '''

#     data_df['delta_acc_lon'] = [None] * len(data_df['challenger y'])

#     for index, row in data_df.iterrows():
#         #if (data_df['challenger y'][index] > data_df['VUT y'][index]):
#         if (calculate_who_is_in_front(vut, challenger) == 'challenger'):
#             lead_vel = data_df['challenger lon acc'][index]
#             follow_vel = data_df['VUT lon acc'][index]
#         else:
#             lead_vel = data_df['VUT lon acc'][index]
#             follow_vel = data_df['challenger lon acc'][index]

#         delta_acc = follow_vel - lead_vel
#         data_df['delta_acc_lon'][index] = delta_acc

def calculate_delta_acc_lon(vut, challenger):
    '''
    calculate delta in acceleration
    delta_acc_lon = a_fol - a_lead
    '''
    ...
    #for index, row in data_df.iterrows():
        #if (data_df['challenger y'][index] > data_df['VUT y'][index]):
    if (calculate_who_is_in_front(vut, challenger) == 'challenger'):
        lead_vel = challenger.acceleration
        follow_vel = vut.acceleration
    else:
        lead_vel = vut.acceleration
        follow_vel = challenger.acceleration
...
def draw_scenario(timestamp, vut, challenger, x_lim_min, x_lim_max, y_lim_min, y_lim_max):
    ...
    sceneInfo = ad.rss.state.UnstructuredSceneStateInformation()
    rss_debug = ad.rss.unstructured.DebugDrawing()
    print(dir(sceneInfo.brakeTrajectorySet))
    print(sceneInfo.brakeTrajectorySet.count)
    for distance in sceneInfo.brakeTrajectorySet:
    ...
    # MM BBoxes
    x1,y1 = vut.mm_bbox.exterior.xy
    x2,y2 = challenger.mm_bbox.exterior.xy

    plt.plot(x1, y1)
    plt.plot(x2, y2)

    # Ped BBox
    x1,y1 = challenger.ped_bbox.exterior.xy

    plt.plot(x1, y1)

    # Vehicle RSS BBox
    x1,y1 = vut.rss_bbox.exterior.xy

    plt.plot(x1, y1)

    # Path polygon

    """ x1,y1 = vut.path_polygon().exterior.xy
    x2,y2 = challenger.path_polygon().exterior.xy
    plt.plot(x1, y1)
    plt.plot(x2, y2) """
...
def process_log(data_df):
    ...
    ##rss_average = RssParameters(alphaLon_accelMax = 5.9, alphaLon_brakeMax = 9.5, alphaLon_brakeMin = 4.1, responseTime = 1.9)
    ...
    ###rss_pedestrian = RssParameters(alphaLon_accelMax=0.0728, alphaLon_brakeMax=0.0728, alphaLon_brakeMin=0.0364, responseTime=0.75, alphaLat_accelMax=0.0273, alphaLat_accelMin=0.01366)
    
    rss_average = RssParameters(alphaLon_accelMax = 2.30, alphaLon_brakeMax = 2.70, alphaLon_brakeMin = 1.78, responseTime = 0.75, alphaLat_accelMin = 1.3435, alphaLat_accelMax = 2.79)
    rss_pedestrian = RssParameters(alphaLon_accelMax=0.76, alphaLon_brakeMax=0.66, alphaLon_brakeMin=0.33, responseTime=0.75, alphaLat_accelMax=0.79, alphaLat_accelMin=0.3545)
    #rss_pedestrian = rss_average
    ## Vehicle information
    vut = vehicle(0, 0, 0, [5.1816, 2.0066], 'vehicle')
    challenger = vehicle(0, 0, 0, [0.4, 0.7], 'pedestrian')
    #challenger = vehicle(0, 0, 0, [4.6, 1.8], 'vehicle')
    ...
    calculate_delta_vel_lon(vut, challenger)
    calculate_delta_acc_lon(vut, challenger)
    ...
    for index, row in data_df.iterrows():
        ...
        vut.mm_bbox = vut.get_mm_bbox()
        vut.rss_bbox = vut.get_rss_vehicle_bbox()
        vut.speed = row['VUT sp']
        vut.acceleration = row['VUT lon acc']

        print(vut.acceleration)
        ...
        challenger.mm_bbox = challenger.get_mm_bbox()
        challenger.ped_bbox = challenger.get_rss_ped_bbox()
        challenger.speed = row['challenger sp']
        challenger.acceleration = row['challenger lon acc']
        ...
        if (sm.d_lat(vut, challenger)[0] == 'left'):
            ...
            left_r = rss_average.responseTime
            leftmal = rss_average.alphaLat_accelMax
            leftdal = rss_average.alphaLat_accelMin
            rightr = rss_pedestrian.responseTime
            rightmal = rss_pedestrian.alphaLat_accelMax
            rightdal = rss_pedestrian.alphaLat_accelMin
        else:
            left_lat_sp = row['challenger lat sp']
            right_lat_sp = row['VUT lat sp']
            left_r = rss_pedestrian.responseTime
            leftmal = rss_pedestrian.alphaLat_accelMax
            leftdal = rss_pedestrian.alphaLat_accelMin
            rightr = rss_average.responseTime
            rightmal = rss_average.alphaLat_accelMax
            rightdal = rss_average.alphaLat_accelMin
        ...
        SEI = sm.calculate_sei(vut=vut,
                            ...
                            left_rho=left_r,
                            left_max_accel_lat=leftmal,
                            left_min_decel_lat=leftdal,
                            right_sp_lat= right_lat_sp,
                            right_rho=rightr,
                            right_max_accel_lat=rightmal,
                            right_min_decel_lat=rightdal,
                            )
        
        #print(SEI[5])
        
        SEI_TTC = sm.calculate_sei_ttc(vut=vut, challenger=challenger)
        SEI_TTC_TWO = sm.calculate_sei_ttc(vut=challenger, challenger=vut)

        #print(SEI_TTC_TWO)

        columns_dict['SEI_TTC'].append(SEI_TTC[0])

        SEI_MM = sm.calculate_sei_mm(vut=vut, challenger=challenger)

        columns_dict['SEI_MM'].append(SEI_MM)
        
        SEI_RSS_PED = sm.calculate_sei_rss_ped(vut=vut, challenger=challenger)
        columns_dict['SEI_RSS_PED'].append(SEI_RSS_PED)

        #print(SEI_MM)
        #SEI[0] = SEI_RSS_PED

        #------------------------
        rss_proper_response = timeUpdate(vut, challenger, timeIndex)
        timeIndex += 1

        SEI[0] = int(not rss_proper_response.isSafe)

        #------------------------

        #SEI[0] = SEI_MM

        # PET = sm.calculate_pet_curve(df=data_df, ego_center_x=data_df['VUT x'], ego_center_y=data_df['VUT y'], ego_yaw=data_df['VUT heading'], ego_length=vut.dimensions[0],
        #                              other_center_x=data_df['challenger x'], other_center_y=data_df['challenger y'], other_yaw=data_df['challenger heading'],
        #                              other_length=challenger.dimensions[0], timestamp=data_df['timestamp'], pet_max=10)
        
        # columns_dict['PET'].append(PET)
        
        # ttc = sm.calculate_ttc(calculate_delta_pos_lon(vut, challenger),calculate_delta_vel_lon(vut, challenger), ttc_max=10)
        # #print(data_df['delta_pos_lon'][index])
        # #print(ttc)
        # if (ttc < 5):
        #     #print('Begin SEI, TTC is less than 5s')
        #     SEI[0] = 1
        # else:
        #     SEI[0] = 0
        ...
        # mttc = sm.calculate_mttc(calculate_delta_pos_lon(vut, challenger),calculate_delta_vel_lon(vut, challenger), calculate_delta_acc_lon(vut, challenger), mttc_max=10)
        # #print(data_df['delta_pos_lon'][index])
        # print(mttc)
        # if (mttc < 5):
        #     SEI[0] = 1
    ...
    overall_da_score = (max(1 - sum([da_score_dict['SEV'] * da_score_dict['SEVM'], 
...
def calculate_safety_metrics(data_path):
    ...
    for file in input_files:
        ...
        scenario_name = sma.get_record_name_from_record(file)
